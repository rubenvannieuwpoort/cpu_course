...
!!Add result and destination register to execute output
...
!!Set default output for invalid or inactive instructions in execute stage

Now, we're ready to actually handle the addition operation in the execute stage.

!!Implement addition operation in execute stage

We can make the "this should never happen" a bit more robust by making it an assertion.

!!Assert false for unhandled operations in execute stage
...
!!Rename decode.vhd to decode_write.vhd and remove write.vhd

Then, I'll make the `decode_write` module take the output from the memory stage.

!!Merge decode and write stages
...
!!Test with two successive instructions
...
!!Propagate is_active to write stage
